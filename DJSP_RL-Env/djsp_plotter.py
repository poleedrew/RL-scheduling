...
class Plotter(object):
    ...
    def plot_job_to_schedule(self, fig_name):
        cmap = plt.cm.get_cmap('Set1', self.num_job_type + 1)
        ...
        labels = ['Job Type {}'.format(t) for t in range(self.num_job_type)]
    ...
    def plot_scheduling_process(self, fig_dir):
        ...
        for timestep in range(1, len(self.logger.history)):
            ...
            cmap = plt.cm.get_cmap('Set1', self.num_job_type + 1)
            yticks = [(i + 1) * 5 for i in range(self.num_machine)]
            fig, ax = plt.subplots(figsize=(16, 8))
            ax.set_ylim(0, 5 * self.num_machine + 5)
            ax.set_yticks(yticks)
            ax.set_yticklabels(["machine {}".format(i) for i in range(self.num_machine)])
    ...
    def plot_interactive_gantt(self, htmlname, has_annotations=False):
        ### timeline
        df = self.logger.to('interactive_gantt_input')
        fig = px.timeline(
            df, x_start='Start', x_end='Finish', y='machine_id', color='job_type', 
            hover_name='Order', hover_data=['job_id', 'op_id', 'RPT'])
